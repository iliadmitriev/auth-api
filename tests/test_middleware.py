...
async def test_error_middleware_400():
    with mock.patch('app.middlewares.handle_http_error', handle_http_error):
        res = await middlewares.error_middleware(request=errors.BadRequest, handler=handler_func)
    assert res == 400


async def test_error_middleware_404():
    with mock.patch('app.middlewares.handle_http_error', handle_http_error):
        res = await middlewares.error_middleware(request=errors.RecordNotFound, handler=handler_func)
    assert res == 404


async def test_error_middleware_403():
    with mock.patch('app.middlewares.handle_http_error', handle_http_error):
        res = await middlewares.error_middleware(request=errors.UserIsNotActivated, handler=handler_func)
    assert res == 403
